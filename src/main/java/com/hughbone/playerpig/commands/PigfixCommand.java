        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, regirstryAccess, environment) -> {
            dispatcher.register(CommandManager.literal("pigfix")
                    ...
                    .executes(ctx -> {
                        ...
                        List<Entity> eList = player.getEntityWorld().getOtherEntities(player, player.getBoundingBox().expand(4, 4, 4));
                        for (Entity entity : eList) {
                            if (entity.getType().equals(EntityType.PIG)) {
                                ...
                                if (((PlayerPigExt) nearbyPig).isPlayerPig()) {
                                    ...
                                    ctx.getSource().getPlayer().sendMessage(Text.of("[PlayerPig] Piggy removed successfully."), false);
                                    return 1;
                                }
                            }
                        }
